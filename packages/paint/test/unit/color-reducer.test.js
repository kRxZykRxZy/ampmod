...
for (const [colorReducer, changeColor, colorProp] of [
    ...
    test('changeColor', () => {
        ...
        // 3 value hex code
        let newColor = '#fff';
        expect(colorReducer(defaultState /* state */, changeColor(newColor) /* action */).primary).toEqual(newColor);
        expect(
            colorReducer(
                {
                    primary: '#010',
    ...
    test('changeColorViaSelectedItems', () => {
        ...
        expect(colorReducer(defaultState /* state */, setSelectedItems(selectedItems) /* action */).primary).toEqual(
            color1
        );
    ...
    test('invalidChangeColor', () => {
        ...
        expect(colorReducer(origState /* state */, changeColor() /* action */)).toBe(origState);
        expect(colorReducer(origState /* state */, changeColor('#') /* action */)).toBe(origState);
        expect(colorReducer(origState /* state */, changeColor('#1') /* action */)).toBe(origState);
        expect(colorReducer(origState /* state */, changeColor('#12') /* action */)).toBe(origState);
        ...
        //     .toBe(origState);
        expect(colorReducer(origState /* state */, changeColor('invalid argument') /* action */)).toBe(origState);
    });
}
