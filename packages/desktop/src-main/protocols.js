...
const FILE_SCHEMES = {
    ...
    "tw-examples": {
        root: path.resolve(__dirname, "../dist-renderer-webpack/examples"),
        standard: true,
        secure: true,
    },
    "tw-packager": {
    ...
    return async request => {
        ...
        try {
            ...
            if (metadata.brotli) {
                ...
                const brotliResponse = await net.fetch(
                    nodeURL.pathToFileURL(`${resolved}.br`)
                );
    ...
    return (request, callback) => {
        const returnErrorResponse = (error, errorPage) => {
            ...
            callback({
                ...
                path: path.join(
                    __dirname,
                    `../src-protocol-error/legacy-file/${errorPage}.html`
                ),
                headers: {
                    ...baseHeaders,
                    ...errorPageHeaders,
        ...
        if (protocol.handle) {
            ...
            if (metadata.brotli) {
                protocol.registerBufferProtocol(
                    scheme,
                    createLegacyBrotliProtocolHandler(metadata)
                );
            } else {
                protocol.registerFileProtocol(
                    scheme,
                    createLegacyFileProtocolHandler(metadata)
                );
            }
        }
    }
