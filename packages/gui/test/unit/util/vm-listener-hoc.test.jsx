...
describe('VMListenerHOC', () => {
    ...
    test('vm green flag event is bound to the passed in prop callback', () => {
        ...
        mount(<WrappedComponent store={store} vm={vm} onGreenFlag={onGreenFlag} />);
    ...
    test('onGreenFlag is not passed to the children', () => {
        const Component = () => <div />;
        const WrappedComponent = vmListenerHOC(Component);
        const wrapper = mount(<WrappedComponent store={store} vm={vm} onGreenFlag={jest.fn()} />);
    ...
    test('targetsUpdate event from vm triggers targets update action', () => {
        const Component = () => <div />;
        const WrappedComponent = vmListenerHOC(Component);
        mount(<WrappedComponent store={store} vm={vm} />);
        const targetList = [];
    ...
    test('keypresses go to the vm', () => {
        ...
        mount(<WrappedComponent attachKeyboardEvents store={store} vm={vm} />);

        // keyboard events that do not target the document or body are ignored
        eventTriggers.keydown({key: 'A', target: null});
        expect(vm.postIOData).not.toHaveBeenLastCalledWith('keyboard', {
            key: 'A',
            isDown: true
        });
        ...
        expect(vm.postIOData).toHaveBeenLastCalledWith('keyboard', {
            key: 'A',
            isDown: true
        });
