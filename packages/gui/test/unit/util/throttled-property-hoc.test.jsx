...
describe('VMListenerHOC', () => {
    ...
    beforeEach(() => {
        const Component = ({propToThrottle, doNotThrottle}) => <input name={doNotThrottle} value={propToThrottle} />;
        const WrappedComponent = ThrottledPropertyHOC('propToThrottle', throttleTime)(Component);

        global.Date.now = () => 0;

        mounted = mount(<WrappedComponent doNotThrottle="oldvalue" propToThrottle={0} />);
    });
