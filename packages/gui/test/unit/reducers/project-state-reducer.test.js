...
test(
    'onLoadedProject(LOADING_VM_WITH_ID, false, false), with project id, ' + 'results in state SHOWING_WITH_ID',
    () => {
        const initialState = {
            projectId: '100',
            loadingState: LoadingState.LOADING_VM_WITH_ID
...
// showing the original project.
test(
    'onLoadedProject(LOADING_VM_WITH_ID, false, false), with no project id, ' + 'results in state SHOWING_WITHOUT_ID',
    () => {
        const initialState = {
            projectId: '0',
            loadingState: LoadingState.LOADING_VM_WITH_ID
...
test(
    'onLoadedProject(LOADING_VM_FILE_UPLOAD, false, false), when we know project id, ' +
...
test(
    'onLoadedProject(LOADING_VM_FILE_UPLOAD, false, false), when we ' +
...
test('calling setProjectId, using with same id as already showing, ' + 'results in state SHOWING_WITH_ID', () => {
    const initialState = {
        projectId: '100',
...
test(
    'calling setProjectId, using different id from project already showing, ' + 'results in state FETCHING_WITH_ID',
    () => {
        const initialState = {
            projectId: 99,
            loadingState: LoadingState.SHOWING_WITH_ID
...
test(
    'setProjectId, with same id as before, but not same type, ' +
...
test(
    'requestNewProject, when can create/save, results in UPDATING_BEFORE_NEW ' +
...
test(
    'projectError when creating new while viewing project with id should ' + 'go back to state SHOWING_WITH_ID',
    () => {
        const initialState = {
            error: null,
            loadingState: LoadingState.CREATING_NEW,
...
test(
    'projectError when creating new while logged out, looking at default project ' +
...
test('projectError encountered while in state FETCHING_WITH_ID results in ' + 'ERROR state', () => {
    const initialState = {
        error: null,
        projectId: null,
        loadingState: LoadingState.FETCHING_WITH_ID
