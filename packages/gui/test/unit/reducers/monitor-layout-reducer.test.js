...
test('initialState', () => {
    ...
    expect(
        monitorLayoutReducer(defaultState /* state */, {type: 'anything'} /* action */).savedMonitorPositions
    ).toBeDefined();
});
...
test('invalidRect', () => {
    ...
    expect(monitorLayoutReducer(reduxState, addMonitorRect(1, {x: 100, y: 100}, {x: 10, y: 200}))).toEqual(reduxState);

    // Problem: y end is before y start
    expect(monitorLayoutReducer(reduxState, addMonitorRect(1, {x: 100, y: 100}, {x: 200, y: 10}))).toEqual(reduxState);
});
...
test('invalidAddMonitorRect', () => {
    ...
    expect(monitorLayoutReducer(reduxState, addMonitorRect(monitorId, upperStart, lowerEnd))).toEqual(reduxState);
});
...
test('invalidMoveMonitorRect', () => {
    ...
    expect(monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 1 /* newX */, 1 /* newY */))).toEqual(
        reduxState
    );
    ...
    // Invalid newX
    expect(monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 'Oregon' /* newX */, 1 /* newY */))).toEqual(
        reduxState
    );

    // Invalid newY
    expect(monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 1 /* newX */))).toEqual(reduxState);
});
...
test('resizeMonitorRect', () => {
    ...
    const reduxState2 = monitorLayoutReducer(reduxState, resizeMonitorRect(monitorId, newWidth, newHeight));
    expect(reduxState2.monitors[monitorId]).toBeDefined();
...
test('invalidResizeMonitorRect', () => {
    ...
    expect(monitorLayoutReducer(reduxState, resizeMonitorRect(monitorId, 1 /* newWidth */, 1 /* newHeight */))).toEqual(
        reduxState
    );
    ...
    // Invalid newWidth
    expect(
        monitorLayoutReducer(reduxState, resizeMonitorRect(monitorId, 'Oregon' /* newWidth */, 1 /* newHeight */))
    ).toEqual(reduxState);

    // Invalid newHeight
    expect(monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 1 /* newWidth */))).toEqual(reduxState);

    // newWidth < 0
    expect(
        monitorLayoutReducer(reduxState, resizeMonitorRect(monitorId, -1 /* newWidth */, 1 /* newHeight */))
    ).toEqual(reduxState);
...
test('invalidRemoveMonitorRect', () => {
    ...
    expect(monitorLayoutReducer(reduxState, resizeMonitorRect(1))).toEqual(reduxState);
});
...
test('getInitialPosition_lineUpTopLeft', () => {
    ...
    reduxState = monitorLayoutReducer(defaultState, addMonitorRect(2, {x: 0, y: height + PADDING}, {x: width, y: 100}));
