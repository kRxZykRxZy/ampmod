...
const {clickText, clickXpath, findByText, findByXpath, getDriver, getLogs, loadUri, rightClickText, scope} =
    new SeleniumHelper();
...
describe('Working with costumes', () => {
    ...
    test('Adding a letter costume through the Letters filter in the library', async () => {
        await loadUri(uri);
        await driver.manage().window().setSize(1244, 768); // Letters filter not visible at 1024 width
        await clickText('Costumes');
        await clickXpath('//button[@aria-label="Choose a Costume"]');
    ...
    test('Load an invalid svg from scratch3 as costume', async () => {
        // eslint-disable-line no-disabled-tests
        await loadUri(uri);
        await clickText('Costumes');
        const el = await findByXpath('//button[@aria-label="Choose a Costume"]');
        await driver.actions().mouseMove(el).perform();
