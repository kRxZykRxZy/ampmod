...
const {clickText, clickXpath, findByText, findByXpath, getDriver, loadUri, rightClickText, scope} =
    new SeleniumHelper();
...
describe('Menu bar settings', () => {
    ...
    test('User is not warned before uploading project file over a fresh project', async () => {
        ...
        const input = await findByXpath('//input[@accept=".sb,.sb2,.sb3,.apz"]');
    ...
    test('Menu labels shown when width is greater than 1024', async () => {
        await loadUri(uri);
        await driver.manage().window().setSize(1200, 768);
