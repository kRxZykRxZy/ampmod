...
test('clear', t => {
    ...
    fontManager.addCustomFont(
        'Wingdings',
        'monospace',
        storage.createAsset(storage.AssetType.Font, 'ttf', new Uint8Array([11, 12, 13]), null, true)
    );
    changed = false;
...
test('custom fonts', t => {
    ...
    fontManager.addCustomFont(
        'Arial',
        'sans-serif',
        storage.createAsset(storage.AssetType.Font, storage.DataFormat.TTF, new Uint8Array([1, 2, 3]), null, true)
    );
    ...
    changed = false;
    setCustomFontsCalls.length = 0;
    const asset = storage.createAsset(storage.AssetType.Font, 'woff2', new Uint8Array([4, 5, 6]), null, true);
    ...
    t.same(setCustomFontsCalls, [
        {
            ...
            // eslint-disable-next-line max-len
            '"Comic Sans MS", serif':
                '@font-face { font-family: "Comic Sans MS"; src: url("data:font/ttf;base64,BAUG"); }'
        }
    ]);
...
test('custom font validation', t => {
    ...
    t.throws(() => {
        fontManager.addCustomFont(
            'family;',
            'sans-serif',
            storage.createAsset(storage.DataFormat.Font, storage.DataFormat.TTF, new Uint8Array([1]), null, true)
        );
    });
...
test('deleteFont', t => {
    ...
    fontManager.addCustomFont(
        'Noto Sans Mono',
        'monospace',
        storage.createAsset(storage.AssetType.Font, storage.DataFormat.TTF, new Uint8Array([17, 18, 19]), null, true)
    );
...
test('overriding existing fonts', t => {
    ...
    const asset = storage.createAsset(storage.AssetType.Font, 'ttf', new Uint8Array([11, 12, 13]), null, true);
