...
test('clone-cleanup', t => {
    ...
    const verifyCounts = (expectedClones, extraThreads) => {
        ...
        t.strictEqual(vm.runtime.targets.length, 2 + expectedClones, `target count at step ${testStep}`);

        // the stage should never have any clones
        t.strictEqual(vm.runtime.targets[0].sprite.clones.length, 1, `stage clone count at step ${testStep}`);
        ...
        t.strictEqual(vm.runtime.threads.length, extraThreads + 2 * expectedClones, `thread count at step ${testStep}`);
    };
