...
compilerAndInterpreter('edge activated hat thread runs after being added to previously executed target', (t, co) => {
    ...
    t.doesNotThrow(() => {
        ...
        vm.loadProject(project).then(() => {
            ...
            threads[0].target.blocks.createBlock(Object.assign({}, hatBlock, {id: 'hatblock2', next: null}));
    ...
    t.doesNotThrow(() => {
        ...
        vm.loadProject(projectWithSprite).then(() => {
            ...
            let numTargetEdgeHats = vm.runtime.targets.reduce(
                (val, target) => val + Object.keys(target._edgeActivatedHatValues).length,
                0
            );
...
compilerAndInterpreter('edge activated hat thread does not interrupt stack click thread', (t, co) => {
    ...
    t.doesNotThrow(() => {
        ...
        vm.loadProject(project).then(() => {
            ...
            vm.runtime.toggleScript(doneThreads[0].topBlock, {
                stackClick: true
            });
...
compilerAndInterpreter('edge activated hat thread does not interrupt stack click thread', (t, co) => {
    ...
    t.doesNotThrow(() => {
        ...
        vm.loadProject(project).then(() => {
            ...
            // Add stack click thread on this hat
            vm.runtime.toggleScript(vm.runtime.threads[0].topBlock, {
                stackClick: true
            });
