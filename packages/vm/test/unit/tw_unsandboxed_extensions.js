...
test('rewriteExtensionURL', async t => {
    ...
    t.notOk(
        vm.extensionManager.isExtensionURLLoaded('https://turbowarp.org/rewritten.js'),
...
test('canDownload', async t => {
    ...
    t.same(canDownloadChecks, [
        ['https://example.com/safe.txt', 'safe.txt'],
        ['https://example.com/unsafe.txt', 'safe.txt'],
        ['https://example.com/safe.txt', 'unsafe.txt'],
        ['https://example.com/unsafe.txt', 'unsafe.txt']
    ]);
...
test('download', async t => {
    ...
    t.same(actualDownloadAttempts, [['https://example.com/safe.txt', 'safe.txt']]);
