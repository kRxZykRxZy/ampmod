        ...
        targets: [
            {
                ...
                blocks: {
                    'CmRa^i]o}QL77;hk:54o': {
                        ...
                        parent: null,
                        inputs: {
                            BACKDROP: [3, 'cq84G6uywD{m2R,E03Ci', 'E3/*4H*xk38{=*U;bVWm']
                        },
                        fields: {},
                        shadow: false,
                        topLevel: true,
...
test('Compressed IDs will not collide with uncompressed IDs', t => {
    ...
    for (const [variableSoup, listSoup, broadcastSoup] of items) {
            ...
            targets: [
                {
                    isStage: true,
                    name: 'Stage',
                    variables: Object.fromEntries(variableSoup.split('').map(id => [id, [id, 0]])),
                    lists: Object.fromEntries(listSoup.split('').map(id => [id, [id, []]])),
                    broadcasts: Object.fromEntries(broadcastSoup.split('').map(id => [id, id])),
                    blocks: {
                        'CmRa^i]o}QL77;hk:54o': {
                            opcode: 'looks_switchbackdropto',
                            next: null,
        ...
        const compressedIDs = [...Object.keys(data.targets[0].blocks), ...Object.keys(data.targets[0].comments)];
...
test('Script execution order is preserved', t => {
    ...
    for (const blockId of blockIds) {
        originalBlocks[blockId] = {
            ...
            inputs: {},
            fields: {
                BROADCAST_OPTION: [`broadcast-name-${blockId}`, `broadcast-id-${blockId}`]
            },
            shadow: false,
            topLevel: true,
            x: -10,
