...
const runServiceTest = function (serviceName, t) {
    ...
    promises.push(
        dispatch.call(serviceName, 'returnFortyTwo').then(
            x => t.equal(x, 42),
            e => t.fail(e)
        )
    );
    ...
    promises.push(
        dispatch.call(serviceName, 'doubleArgument', 9).then(
            x => t.equal(x, 18),
            e => t.fail(e)
        )
    );
    ...
    promises.push(
        dispatch.call(serviceName, 'doubleArgument', 123).then(
            x => t.equal(x, 246),
            e => t.fail(e)
        )
    );
    ...
    promises.push(
        dispatch.call(serviceName, 'throwException').then(
...
test('local', t => {
    dispatch.setService('LocalDispatchTest', new DispatchTestService()).catch(e => t.fail(e));
...
test('remote', t => {
    ...
    const waitForWorker = new Promise(resolve => {
        dispatch.setService('test', {onWorkerReady: resolve}).catch(e => t.fail(e));
    });

    return waitForWorker
        .then(
            () => runServiceTest('RemoteDispatchTest', t),
            e => t.fail(e)
        )
        .then(
            () => dispatch._remoteCall(worker, 'dispatch', 'terminate'),
            e => t.fail(e)
        );
});
