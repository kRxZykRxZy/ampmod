...
import {IntermediateStack, IntermediateInput, IntermediateScript, IntermediateRepresentation, IntermediateStackBlock} from './intermediate.js';
...
class TypeState {
    ...
    setVariableType (variable, type) {
        if (this.variables[variable.id] === type) {
            return false;
        }
...
class IROptimizer {
    ...
    getInputType (inputBlock, state) {
        ...
        case InputOpcode.CAST_NUMBER_OR_NAN: {
            const innerType = inputs.target.type;
            if (innerType & InputType.NUMBER_OR_NAN) {
                return innerType;
            }
        ...
        case InputOpcode.OP_ADD: {
            ...
            const canBeNaN = function () {
                ...
                if (leftType & InputType.NUMBER_POS_INF && rightType & InputType.NUMBER_NEG_INF) {
                    return true;
                }
                // (-Infinity) + Infinity = NaN
                if (leftType & InputType.NUMBER_NEG_INF && rightType & InputType.NUMBER_POS_INF) {
                    return true;
                }
            };
            if (canBeNaN()) {
                resultType |= InputType.NUMBER_NAN;
            }
            const canBeFractional = function () {
                ...
                if (leftType & InputType.NUMBER_FRACT) {
                    return true;
                }
                if (rightType & InputType.NUMBER_FRACT) {
                    return true;
                }
            };
            const canBeFract = canBeFractional();
            const canBePos = function () {
                if (leftType & InputType.NUMBER_POS) {
                    return true;
                } // POS + ANY ~= POS
                if (rightType & InputType.NUMBER_POS) {
                    return true;
                } // ANY + POS ~= POS
            };
            if (canBePos()) {
                ...
                if (canBeFract) {
                    resultType |= InputType.NUMBER_POS_FRACT;
                }
            }
            const canBeNeg = function () {
                if (leftType & InputType.NUMBER_NEG) {
                    return true;
                } // NEG + ANY ~= NEG
                if (rightType & InputType.NUMBER_NEG) {
                    return true;
                } // ANY + NEG ~= NEG
            };
            if (canBeNeg()) {
                ...
                if (canBeFract) {
                    resultType |= InputType.NUMBER_NEG_FRACT;
                }
            }
            const canBeZero = function () {
                // POS_REAL + NEG_REAL ~= 0
                if (leftType & InputType.NUMBER_POS_REAL && rightType & InputType.NUMBER_NEG_REAL) {
                    return true;
                }
                // NEG_REAL + POS_REAL ~= 0
                if (leftType & InputType.NUMBER_NEG_REAL && rightType & InputType.NUMBER_POS_REAL) {
                    return true;
                }
                // 0 + 0 = 0
                if (leftType & InputType.NUMBER_ZERO && rightType & InputType.NUMBER_ZERO) {
                    return true;
                }
                // 0 + -0 = 0
                if (leftType & InputType.NUMBER_ZERO && rightType & InputType.NUMBER_NEG_ZERO) {
                    return true;
                }
                // -0 + 0 = 0
                if (leftType & InputType.NUMBER_NEG_ZERO && rightType & InputType.NUMBER_ZERO) {
                    return true;
                }
            };
            if (canBeZero()) {
            ...
            const canBeNegZero = function () {
                // -0 + -0 = -0
                if (leftType & InputType.NUMBER_NEG_ZERO && rightType & InputType.NUMBER_NEG_ZERO) {
                    return true;
                }
            };
            if (canBeNegZero()) {
                resultType |= InputType.NUMBER_NEG_ZERO;
            }
            return resultType;
        }
        case InputOpcode.OP_SUBTRACT: {
            ...
            const canBeNaN = function () {
                // Infinity - Infinity = NaN
                if (leftType & InputType.NUMBER_POS_INF && rightType & InputType.NUMBER_POS_INF) {
                    return true;
                }
                // (-Infinity) - (-Infinity) = NaN
                if (leftType & InputType.NUMBER_NEG_INF && rightType & InputType.NUMBER_NEG_INF) {
                    return true;
                }
            };
            if (canBeNaN()) {
            ...
            const canBeZero = function () {
                // POS_REAL - POS_REAL ~= 0
                if (leftType & InputType.NUMBER_POS_REAL && rightType & InputType.NUMBER_POS_REAL) {
                    return true;
                }
                // NEG_REAL - NEG_REAL ~= 0
                if (leftType & InputType.NUMBER_NEG_REAL && rightType & InputType.NUMBER_NEG_REAL) {
                    return true;
                }
                // 0 - 0 = 0
                if (leftType & InputType.NUMBER_ZERO && rightType & InputType.NUMBER_ZERO) {
        ...
        case InputOpcode.OP_MULTIPLY: {
            ...
            const canBeNaN = function () {
                // (-)Infinity * 0 = NaN
                if (leftType & InputType.NUMBER_INF && rightType & InputType.NUMBER_ANY_ZERO) {
                    return true;
                }
                // 0 * (-)Infinity = NaN
                if (leftType & InputType.NUMBER_ANY_ZERO && rightType & InputType.NUMBER_INF) {
                    return true;
                }
            };
            if (canBeNaN()) {
            ...
            const canBePos = function () {
                // POS * POS = POS
                if (leftType & InputType.NUMBER_POS && rightType & InputType.NUMBER_POS) {
                    return true;
                }
                // NEG * NEG = POS
                if (leftType & InputType.NUMBER_NEG && rightType & InputType.NUMBER_NEG) {
                    return true;
                }
            };
            if (canBePos()) {
            ...
            const canBeNeg = function () {
                // POS * NEG = NEG
                if (leftType & InputType.NUMBER_POS && rightType & InputType.NUMBER_NEG) {
                    return true;
                }
                // NEG * POS = NEG
                if (leftType & InputType.NUMBER_NEG && rightType & InputType.NUMBER_POS) {
                    return true;
                }
            };
            if (canBeNeg()) {
            ...
            const canBeZero = function () {
                ...
                // 0 * POS_REAL = 0
                if (leftType & InputType.NUMBER_ZERO && rightType & InputType.NUMBER_POS_REAL) {
                    return true;
                }
                // -0 * NEG_REAL = 0
                if (leftType & InputType.NUMBER_NEG_ZERO && rightType & InputType.NUMBER_NEG_REAL) {
                    return true;
                }
                // POS_REAL * 0 = 0
                if (leftType & InputType.NUMBER_POS_REAL && rightType & InputType.NUMBER_ZERO) {
                    return true;
                }
                // NEG_REAL * -0 = 0
                if (leftType & InputType.NUMBER_NEG_REAL && rightType & InputType.NUMBER_NEG_ZERO) {
                    return true;
                }
                // Rounding errors like 1e-323 * 0.1 = 0
                if (leftType & InputType.NUMBER_FRACT && rightType & InputType.NUMBER_FRACT) {
                    return true;
                }
            };
            if (canBeZero()) {
            ...
            const canBeNegZero = function () {
                ...
                // -0 * POS_REAL = -0
                if (leftType & InputType.NUMBER_NEG_ZERO && rightType & InputType.NUMBER_POS_REAL) {
                    return true;
                }
                // 0 * NEG_REAL = -0
                if (leftType & InputType.NUMBER_ZERO && rightType & InputType.NUMBER_NEG_REAL) {
                    return true;
                }
                // POS_REAL * -0 = -0
                if (leftType & InputType.NUMBER_POS_REAL && rightType & InputType.NUMBER_NEG_ZERO) {
                    return true;
                }
                // NEG_REAL * 0 = -0
                if (leftType & InputType.NUMBER_NEG_REAL && rightType & InputType.NUMBER_ZERO) {
                    return true;
                }
        ...
        case InputOpcode.OP_DIVIDE: {
            ...
            const canBeNaN = function () {
                // (-)0 / (-)0 = NaN
                if (leftType & InputType.NUMBER_ANY_ZERO && rightType & InputType.NUMBER_ANY_ZERO) {
                    return true;
                }
                // (-)Infinity / (-)Infinity = NaN
                if (leftType & InputType.NUMBER_INF && rightType & InputType.NUMBER_INF) {
                    return true;
                }
                // (-)0 / NaN = NaN
                if (leftType & InputType.NUMBER_ANY_ZERO && rightType & InputType.NUMBER_NAN) {
                    return true;
                }
            };
            if (canBeNaN()) {
            ...
            if (canBePos()) {
                resultType |= InputType.NUMBER_POS;
            }
            const canBeNegInfinity = function () {
                // NEG / 0 = -Infinity
                if (leftType & InputType.NUMBER_NEG && rightType & InputType.NUMBER_ZERO) {
                    return true;
                }
                // POS / -0 = -Infinity
                if (leftType & InputType.NUMBER_POS && rightType & InputType.NUMBER_NEG_ZERO) {
                    return true;
                }
            ...
            const canBeInfinity = function () {
                // POS / 0 = Infinity
                if (leftType & InputType.NUMBER_POS && rightType & InputType.NUMBER_ZERO) {
                    return true;
                }
                // NEG / -0 = Infinity
                if (leftType & InputType.NUMBER_NEG && rightType & InputType.NUMBER_NEG_ZERO) {
                    return true;
                }
            ...
            if (canBeNeg()) {
                resultType |= InputType.NUMBER_NEG;
            }
            const canBeZero = function () {
                // 0 / POS = 0
                if (leftType & InputType.NUMBER_ZERO && rightType & InputType.NUMBER_POS) {
                    return true;
                }
                // -0 / NEG = 0
                if (leftType & InputType.NUMBER_NEG_ZERO && rightType & InputType.NUMBER_NEG) {
                    return true;
                }
                // Rounding errors like 1e-323 / 10 = 0
                if (leftType & InputType.NUMBER_POS_REAL && rightType & InputType.NUMBER_POS_REAL) {
                ...
                if (leftType & InputType.NUMBER_POS && rightType & InputType.NUMBER_POS_INF) {
                    return true;
                }
                // NUMBER_NEG / -Infinity = 0
                if (leftType & InputType.NUMBER_NEG && rightType & InputType.NUMBER_NEG_INF) {
                    return true;
                }
            };
            if (canBeZero()) {
            ...
            const canBeNegZero = function () {
                // -0 / POS = -0
                if (leftType & InputType.NUMBER_NEG_ZERO && rightType & InputType.NUMBER_POS) {
                    return true;
                }
                // 0 / NEG = -0
                if (leftType & InputType.NUMBER_ZERO && rightType & InputType.NUMBER_NEG) {
                    return true;
                }
                // Rounding errors like -1e-323 / 10 = -0
                if (leftType & InputType.NUMBER_NEG_REAL && rightType & InputType.NUMBER_POS_REAL) {
                ...
                // NUMBER_POS / -Infinity = -0
                if (leftType & InputType.NUMBER_POS && rightType & InputType.NUMBER_NEG_INF) {
                    return true;
                }
                // NUMBER_NEG / Infinity = -0
                if (leftType & InputType.NUMBER_NEG && rightType & InputType.NUMBER_POS_INF) {
                    return true;
                }
            };
            if (canBeNegZero()) {
    ...
    analyzeStack (stack, state) {
        ...
        for (const stackBlock of stack.blocks) {
            ...
            if (!stackBlock.ignoreState) {
                if (stackBlock.yields && !this.ignoreYields) {
                    stateChanged = state.clear() || stateChanged;
                }
                if (stateChanged) {
                    if (stackBlock.exitState) {
                        stackBlock.exitState.or(state);
                    } else {
