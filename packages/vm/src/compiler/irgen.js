...
import compatBlocks from './compat-blocks.js';
...
import oldCompilerCompatiblity from './old-compiler-compatibility.js';
/**
...
class ScriptTreeGenerator {
    ...
    createConstantInput (constant, preserveStrings = false) {
        ...
        if (!Number.isNaN(numConstant) && (constant.trim() !== '' || constant.includes('\t'))) {
            if (!preserve && numConstant.toString() === constant) {
                return new IntermediateInput(InputOpcode.CONSTANT, IntermediateInput.getNumberInputType(numConstant), {
                    value: numConstant
                });
            }
    ...
    descendInput (block, preserveStrings = false) {
        ...
        case 'argument_reporter_boolean': {
            ...
            if (index === -1) {
                if (name.toLowerCase() === 'is compiled?' ||
                        ...
                        /is (ampmod|ultiblocks|turbowarp)\?/.test(name.toLowerCase())) {
        ...
        case 'arrays_range':
            return new IntermediateInput(InputOpcode.ARRAYS_RANGE, InputType.ARRAY, {
        ...
        case 'arrays_item_of':
            return new IntermediateInput(InputOpcode.ARRAYS_INDEX, InputType.ANY, {
                index: this.descendInputOfBlock(block, 'INDEX').toType(InputType.NUMBER),
                array: this.descendInputOfBlock(block, 'VALUE').toType(InputType.ARRAY)
            });
        case 'arrays_length':
            return new IntermediateInput(InputOpcode.ARRAYS_LENGTH, InputType.NUMBER, {
                array: this.descendInputOfBlock(block, 'VALUE').toType(InputType.ARRAY)
            });
        case 'arrays_contains':
            return new IntermediateInput(InputOpcode.ARRAYS_CONTAINS, InputType.BOOLEAN, {
        ...
        case 'arrays_in_front_of':
            return new IntermediateInput(InputOpcode.ARRAYS_BEHIND, InputType.ARRAY, {
                array: this.descendInputOfBlock(block, 'ARRAY').toType(InputType.ARRAY),
                item: this.descendInputOfBlock(block, 'ITEM')
            });
        case 'arrays_behind':
            return new IntermediateInput(InputOpcode.ARRAYS_IN_FRONT_OF, InputType.ARRAY, {
                array: this.descendInputOfBlock(block, 'ARRAY').toType(InputType.ARRAY),
                item: this.descendInputOfBlock(block, 'ITEM')
            });
        ...
        case 'operator_equals':
            return new IntermediateInput(
                ...
                this.runtime.runtimeOptions.caseSensitivity ? InputOpcode.OP_EQUALS_CS : InputOpcode.OP_EQUALS, InputType.BOOLEAN, {
        ...
        case 'operator_mathop': {
            ...
            switch (operator) {
            case 'abs':
                return new IntermediateInput(InputOpcode.OP_ABS, InputType.NUMBER_POS | InputType.NUMBER_ZERO, {
                    value
                });
            case 'floor':
                return new IntermediateInput(InputOpcode.OP_FLOOR, InputType.NUMBER, {value});
            case 'ceiling':
                return new IntermediateInput(InputOpcode.OP_CEILING, InputType.NUMBER, {value});
            case 'sqrt':
                return new IntermediateInput(InputOpcode.OP_SQRT, InputType.NUMBER_OR_NAN, {value});
            case 'sin':
                return new IntermediateInput(InputOpcode.OP_SIN, InputType.NUMBER_OR_NAN, {value});
            case 'cos':
                return new IntermediateInput(InputOpcode.OP_COS, InputType.NUMBER_OR_NAN, {value});
            case 'tan':
                return new IntermediateInput(InputOpcode.OP_TAN, InputType.NUMBER_OR_NAN, {value});
            case 'asin':
                return new IntermediateInput(InputOpcode.OP_ASIN, InputType.NUMBER_OR_NAN, {value});
            case 'acos':
                return new IntermediateInput(InputOpcode.OP_ACOS, InputType.NUMBER_OR_NAN, {value});
            case 'atan':
                return new IntermediateInput(InputOpcode.OP_ATAN, InputType.NUMBER, {value});
            case 'ln':
                return new IntermediateInput(InputOpcode.OP_LOG_E, InputType.NUMBER_OR_NAN, {value});
            case 'log':
                return new IntermediateInput(InputOpcode.OP_LOG_10, InputType.NUMBER_OR_NAN, {value});
            case 'e ^':
                return new IntermediateInput(InputOpcode.OP_POW_E, InputType.NUMBER, {value});
            case '10 ^':
        ...
        case 'sensing_current':
            ...
            case 'year':
                return new IntermediateInput(InputOpcode.SENSING_TIME_YEAR, InputType.NUMBER_POS_REAL | InputType.NUMBER_ZERO);
            case 'month':
                return new IntermediateInput(InputOpcode.SENSING_TIME_MONTH, InputType.NUMBER_POS_REAL);
            case 'date':
                return new IntermediateInput(InputOpcode.SENSING_TIME_DATE, InputType.NUMBER_POS_REAL);
            case 'dayofweek':
                return new IntermediateInput(InputOpcode.SENSING_TIME_WEEKDAY, InputType.NUMBER_POS_REAL);
            case 'hour':
                return new IntermediateInput(InputOpcode.SENSING_TIME_HOUR, InputType.NUMBER_POS_REAL | InputType.NUMBER_ZERO);
            case 'minute':
                return new IntermediateInput(InputOpcode.SENSING_TIME_MINUTE, InputType.NUMBER_POS_REAL | InputType.NUMBER_ZERO);
            case 'second':
                return new IntermediateInput(InputOpcode.SENSING_TIME_SECOND, InputType.NUMBER_POS_REAL | InputType.NUMBER_ZERO);
            default:
                return this.createConstantInput(0);
            }
        ...
        case 'sensing_of': {
            ...
            if (object.isConstant('_stage_')) {
                ...
                case 'direction':
                    return new IntermediateInput(InputOpcode.SENSING_OF_DIRECTION, InputType.NUMBER_REAL, {
                        object
                    });
                case 'costume #':
                    return new IntermediateInput(InputOpcode.SENSING_OF_COSTUME_NUMBER, InputType.NUMBER_POS_REAL, {object});
                case 'costume name':
                    return new IntermediateInput(InputOpcode.SENSING_OF_COSTUME_NAME, InputType.STRING, {
                        object
                    });
                case 'size':
                    return new IntermediateInput(InputOpcode.SENSING_OF_SIZE, InputType.NUMBER_POS_REAL, {
                        object
                    });
                }
            }
...
class IRGenerator {
    ...
    generate () {
        ...
        while (this.proceduresToCompile.size > 0) {
            ...
            this.proceduresToCompile = new Map();
            for (const [procedureVariant, definitionId
            // @ts-ignore
