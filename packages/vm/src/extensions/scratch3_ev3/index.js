...
class EV3Motor {
    ...
    turnOnFor (milliseconds) {
        ...
        byteCommand = byteCommand
            .concat([Ev3Args.LAYER, port, Ev3Encoding.ONE_BYTE, dir & 0xff, Ev3Encoding.ONE_BYTE, rampup])
...
class EV3 {
    ...
    _onMessage (params) {
        ...
        if (this._updateDevices) {
            ...
            for (let i = 0; i < 4; i++) {
                // array 2 float
                const buffer = new Uint8Array([data[offset], data[offset + 1], data[offset + 2], data[offset + 3]])
                    .buffer;
